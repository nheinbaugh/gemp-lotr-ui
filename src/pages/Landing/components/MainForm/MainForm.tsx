...
function MainForm() {
    ...
    const [{ data: loginResponse, loading: isLoginLoading, error: loginError }, executeLogin] = useAxios<string, LoginRequestInterface, Error>('/gemp-lotr-server/login', { manual: true });
    const [{ data: registrationResponse, loading: isRegistrationLoading, error: registrationError }, executeRegister] = useAxios<string, LoginRequestInterface, Error>('/gemp-lotr-server/register', { manual: true });
    ...
    return (
        <Sheet className={styles.form} sx={{ backgroundColor: 'neutral.300', opacity: 0.9, p: 2, width: '30%', height: '40%' }}>
                <Typography level="h3">Lord of the Rings TCG on GEMP</Typography>
                {
                    currentForm === 'registration'
                        ? <Registration onRegister={registerAttempt} error={registrationError} isLoading={isRegistrationLoading} />
                        : <Login
